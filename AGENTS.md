...
- **Key deps**: numpy 2.5+, scipy 1.18+, scikit-image, SimpleITK, Zarr 3.0+, OME-Zarr 0.9+, napari
- **linum-basic / basicpy**: temporarily disabled — the linum-basic repo is being cleaned up and basicpy functions are being ported to linumpy. Install via `linumpy[illumination]` once re-enabled.
